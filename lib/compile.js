var compile = (function(){
    ...
    var csso_util = (new Function('var exports = {};' + readFileSync(__dirname + '/../csso/lib/util.js').toString() + ' return $util;'))();
    var csso_parse = (new Function('var exports = {};' + readFileSync(__dirname + '/../csso/lib/parser.js').toString() + ' return exports.parse;'))();
    var csso_translator = (new Function('var exports = {};' + readFileSync(__dirname + '/../csso/lib/translator.js').toString() + ' return exports.translator;'))();
    var csso_compressor = (new Function('translator', 'clearInfo', 'var exports = {};' + readFileSync(__dirname + '/../csso/lib/compressor.js').toString() + ' return exports.compress;'))(csso_translator, csso_util.cleanInfo);
    var csso = function(src, ro) {
        return csso_translator(csso_util.cleanInfo(csso_compressor(csso_parse(src, 'stylesheet'), ro)));
    };
    ...
    function compileAttributes(attrs){
        var i, result = { 'text': '', 'expr': [] }, n = 0, attrValue = '';
        for (i in attrs){
    ...
    function getAttr(compile_file, parser) {
        return function(node, attr) {
            try {
                attr = node.attributes[attr].value;
            } catch (e) {
    ...
    function _compile (file, context_name, callbacks, options, cb){
        ...
        parser.onopentag = function (node) {
            ...
            for (var i = 0; i < attrs.expr.length; i++) {
                fn[fn.length - 1] += 'try{__fest_attrs[' + i + ']=(' + attrs.expr[i] + ')}catch(e){__fest_attrs[' + i + ']=""; __fest_log_error(e.message);}';
            }
            ...
            switch (node.local){
                ...
                case 'doctype':
                    ...
                    return;
                case 'attribute':
                    fn[fn.length - 1] += '__fest_pushstr(' + context_name + '," ' + _getAttr(node, 'name') + '=\\"");';
                    return;
                case 'comment':
                    fn[fn.length - 1] += '__fest_pushstr(' + context_name + ',"<!--");';
                    return;
                case 'cdata':
                    ...
                    return;
                case 'text':
                    fn[fn.length - 1] += node.attributes.value ? '__fest_pushstr(' + context_name + ',"' + _getAttr(node, 'value') + '");' : '';
                    return;
                case 'space':
                    fn[fn.length - 1] += '__fest_pushstr(' + context_name + '," ");';
                    return;
                case 'if':
                    fn[fn.length - 1] += 'try{__fest_if=' + _getAttr(node, 'test') + '}catch(e){__fest_if=false;__fest_log_error(e.message);}';
                ...
                case 'script':
                    ...
                    return;
                case 'insert':
                    file = readFileSync(dirname(file) + '/' + _getAttr(node, 'src'), 'utf-8');
                    if (node.attributes.type && node.attributes.type === 'type/css'){
                        file = csso(file);
                    }
                    fn[fn.length - 1] += '__fest_pushstr(' + context_name + ',"' + file
                        .replace(/\\/g, '\\\\')
                        .replace(/"/g, '\\"')
                        .replace(/\n/g, '\\n')
                        .replace(/\r/g, '') + '");';
                ...
                case 'for':
                    ...
                    fn[fn.length - 1] += 'try{__fest_foreach=' + _getAttr(node, 'iterate') + ' || {};}catch(e){__fest_foreach={};__fest_log_error(e.message);}';
                ...
                case 'foreach':
                    ...
                    if (node.attributes.iterate){
                        ...
                        fn[fn.length - 1] += 'try{__fest_from' + counters.counter + '=' + node.attributes.from.value + ';}catch(e){__fest_from' + counters.counter + '=0;__fest_log_error(e.message);}';
                        fn[fn.length - 1] += 'try{__fest_to' + counters.counter + '=' + _getAttr(node, 'to') + ';}catch(e){__fest_to' + counters.counter + '=0;__fest_log_error(e.message);}';
                ...
                case 'set':
                    ...
                    fn[fn.length - 1] += 'if(typeof __fest_push === "undefined"){__fest_pushstr=function(ctx,str){__fest_str+=str}}else{__fest_pushstr=__fest_push}';
                    return;
                case 'get':
                ...
                case 'include':
                    if (node.attributes.context){
                        ...
                        fn[fn.length - 1] += 'try{' + context_name + '=' + _getAttr(node, 'context') + '}catch(e){' + context_name + '={};__fest_log_error(e.message)};';
                    }
        ...
        parser.ontext = function (text) {
            ...
            if (evallist.indexOf(stack[stack.length - 1]) === -1){
                fn[fn.length - 1] += '__fest_pushstr(' + context_name + ',"' + text.replace(/\\/g, '\\\\').replace(/\n/g, '\\n').replace(/\r/g, '').replace(/"/g, '\\"') + '");';
        ...
        parser.oncdata = function (cdata) {
            ...
            if (evallist.indexOf(stack[stack.length - 1]) === -1){
                fn[fn.length - 1] += '__fest_pushstr(' + context_name + ',"' + cdata.replace(/\\/g, '\\\\').replace(/\n/g, "\\n").replace(/\r/g, '').replace(/"/g, '\\"') + '");';
            } else if (stack[stack.length - 1] === 'get') {
    ...
    function finish_compile(file, options, name){
        ...
        function build_template(result) {
            ...
            template = 'function ' + name + '(__fest_context){"use strict";' +
                'var __fest_str="",__fest_result=[],__fest_contexts=[],__fest_attrs=[],__fest_if,__fest_foreach,__fest_from,__fest_to,__fest_html = "",' +
