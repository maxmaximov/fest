        ...
        parser.onopentag = function (node) {
            ...
            if (node.ns[node.prefix] != fest_ns){
                ...
                for (var i = 0; i < attrs.expr.length; i++) {
                    fn[fn.length - 1] += 'try{__fest_attrs[' + i + ']=__fest_escapeHTML(' + attrs.expr[i] + ')}catch(e){__fest_attrs[' + i + ']=""; __fest_log_error(e.message);}';
                }
            ...
            switch (node.local){
                case 'element':
                    ...
                    fn[fn.length - 1] += 'try{__fest_elementName=' + _getAttr(node, 'name') + ';';
                ...
                case 'script':
                    ...
                    return;
                case 'call':
                    fn[fn.length - 1] += 'try{__fest_pushstr(' + context_name + ',' + _getAttr(node, 'name') + '(' + _getAttr(node, 'data') + '))}catch(e){__fest_log_error(e.message)}';
                    return;
                case 'insert':
                    fn[fn.length - 1] += '__fest_pushstr(' + context_name + ',"' + readFileSync(dirname(file) + '/' + _getAttr(node, 'src'), 'utf-8')
                        .replace(/\\/g, '\\\\')
                        .replace(/"/g, '\\"')
                        .replace(/\n/g, '\\n')
                        .replace(/\r/g, '') + '");';
                    return;
                case 'each':
                    fn[fn.length - 1] += 'var ' + _getAttr(node, 'index') + (node.attributes.value ? ',' + _getAttr(node, 'value')  : '') + ';';
                ...
                case 'for':
                    fn[fn.length - 1] += 'var ' + _getAttr(node, 'index') + (node.attributes.value ? ',' + _getAttr(node, 'value')  : '') + ',__fest_l' + counters.counter + ',__fest_from' + counters.counter + ',__fest_to' + counters.counter + ';';
                ...
                case 'get':
                    ...
                    if (node.attributes.select){
                        fn[fn.length - 1] += 'try{__fest_select=' + _getAttr(node, 'select') + '}catch(e){__fest_select==="__error__";__fest_log_error(e.message)}';
                    } else {
                        fn[fn.length - 1] += '__fest_select="' + _getAttr(node, 'name') + '";';
                    }
